...
> 通过U+平台提供的接口，同步用户设备列表数据，建议同步机制如下（两种机制使用的是同一个接口）：首次OAuth授权后，第三方平台服务自动触发同步用户设备列表；用户授权后，以下几种情况需要用户在第三方app手动进行同步：在智家APP新绑定了设备；在智家APP解绑了设备；第三方平台增加了线上对接的新的设备型号。
...
> 通过U+平台提供的接口，获取用户设备详情数据；用户设备详情交互界面。
...
> 通过U+平台提供的接口，获取用户设备状态数据；用户设备状态界面交互。
...
> 通过U+平台提供的接口，实现设备控制指令下发，并获取控制结果；用户设备控制界面交互。
