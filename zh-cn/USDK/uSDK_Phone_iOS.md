...
### 4.5管理设备连接状态变化
当设备状态（uSDK与设备连接状态）发生变化时，uPlug模块或云平台会及时的通知uSDK，uSDK会上报收到的设备状态给APP，用于更新UI界面或进行其他业务处理。设备连接状态分为：未连接、连接中、已连接（连接成功）、就绪、离线，五个状态值。关于设备状态值的具体定义，请参考7.1.16 uSDK与设备连接状态值定义<br>
...
标准文档是海尔的主推方向，需要和uSDK3.x或uSDK4.x、标准设备，三方面一起使用才能完成对设备的控制。标准文档的使用需要参考标准文档模型和下面的写属性、操作命令进行开发。uSDK2.x不支持标准文档。<br>
...
name：属性名，NSString类型。如”204003”<br>

value：属性值，NSString类型。如”204003”<br>
...
“单命令/组命令”说明当前指令204001可以既作为单命令发送，也可以运用到组命令中。<br>
...
4.切网：用户由设备所在A路由切换到B路由，或改用数据网络的行为。<br>
...
#### 测试远程功能是否正常
手机在配置设备到A路由，绑定设备成功。手机或平板电脑切换连接B路由或直接使用2G、3G、4G数据网络，查询设备状态、进行控制。<br>
...
### 8.3. 为何设备已经进入待配置模式，手机发送完配置信息后，手机依然处于待配置模式？也许换一个手机就可以正常配置。
答：是由于手机和路由的兼容性不好引起的，打开路由无线网络基本设置界面，修改为如下数据：无线模式：11bg mixed   频段带宽：20MHz 。

### 8.4.	App在等待设备就绪或已连接状态时如何处理 
...
### 8.6.	为什么移动应用业务数据通路和uSDK数据通路需要在同一个环境？
...
### 8.8.模块软件版本大于等于	2.5.00，设备连接后始终离线？
答：模块软件版本大于等于2.5.00的设备是安全类设备，需要完成绑定后才能就绪。

### 8.9	为何绑定失败，提示“非安全设备校验失败”
...
[public_single_cmd]:__https://haier-iot.github.io/guide/zh-cn/__https://haier-iot.github.io/guide/zh-cn/_media/_usdk/public_single_cmd.png
